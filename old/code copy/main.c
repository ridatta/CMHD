		...
		for (int idx=0; idx<nvar; idx++){
			for (int j=1; j<(ny-1); j++){
				for (int i=1; i<(nx-1); i++){
					if (internal_bound[j][i] != 1.0){ // Fluid Region
						...
						//~ Up[0][j][i] = 0.25 * (Up[0][j][i-1] +  Up[0][j][i+1]
						//~ +  Up[0][j-1][i] +  Up[0][j+1][i]);
						Up[0][j][i] = 1.1;
			...
			for (int j=1; j<(ny-1); j++){
				for (int i=1; i<(nx-1); i++){
					if (internal_bound[j][i] != 1.0){ // Fluid Region
						...
						//~ U[0][j][i] = 0.25 * (U[0][j][i-1] +  U[0][j][i+1]
						//~ +  U[0][j-1][i] +  U[0][j+1][i]);
						//~ U[3][j][i] = 1.0 * U[3][j][i];
						U[0][j][i] = 1.1;
