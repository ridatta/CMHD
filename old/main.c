...
#define Ms 2.0
#define pi 3.1415926
#define def_angle 3.1415926/12.0
#define A 0.0
#define alpha 1.0
#define beta 0.5
...
int main(){
	...
	for (int j=0; j<ny; j++){
		for (int i=0; i<nx; i++){
			if (sqrt(i*dx*i*dx + j*dx*j*dx)<0.15){
			...
			for (int j=1; j<(ny-1); j++){
				for (int i=1; i<(nx-1); i++){
					if (internal_bound[j][i] == 1){
						...
						S[2] = 0.0;
						T = p[j][i] * 1.0 / rho[j][i];
						S[4] = -1.0 * A * rho[j][i] * pow(rho[j][i],alpha) * pow(T,beta); // radiation term
